    ...
    <div class="card-header">
      <div class="row align-items-center justify-content-between">
        <div class="col-6 col-sm-auto d-flex align-items-center pr-0">
          <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Goods Store Out </h5>
        </div>
        <div class="col-6 col-sm-auto ml-auto text-right pl-0">
          ...
          <div id="dashboard-actions">
            <a href="{{route('goodstoreout.create')}}" class="btn btn-falcon-primary btn-sm" type="button">
              <span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span>
              <span class="d-none d-sm-inline-block ml-1">New</span>
            </a>

            <a href="{{route('goodstoreoutReport')}}" class="btn btn-falcon-info btn-sm" type="button">
        ...
        <table class="table table-sm table-dashboard fs--1 data-table border-bottom" data-options='{"responsive":false,"pagingType":"simple","lengthChange":true,"searching":true,"pageLength":8,"columnDefs":[{"targets":[0,6],"orderable":false}],"language":{"info":"_START_ to _END_ Items of _TOTAL_  <a href=\"#!\" class=\"font-weight-semi-bold\"> view all <span class=\"fas fa-angle-right\" data-fa-transform=\"down-1\"></span> </a>"},"buttons":["copy","excel"]}'>
          <thead class="bg-200 text-900">
            <tr>
              ...
              <th>Issued Date</th>
              <th>Quantity Issued / Required</th>
              <th>Unit</th>
          ...
          @foreach($goodstoreout as $staff)
            <tr class="btn-reveal-trigger">
              ...
              <th class="align-middle">{{$staff->item_name_show}}</th>
              <td class="align-middle">{{$staff->issue_date}}</td>
              <td class="align-middle">{{$staff->quantity_issued}} / {{$staff->quantity_required}}</td>
                ...
                <div class="dropdown text-sans-serif"><button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal mr-3" type="button" id="dropdown0" data-toggle="dropdown" data-boundary="html" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs--1"></span></button>
                  <div class="dropdown-menu dropdown-menu-right border py-0" aria-labelledby="dropdown0">
                    <div class="bg-white py-2">
                      <a class="dropdown-item" href="{{route('goodstoreout.edit',['goodstoreout'=>$staff->id])}}">Edit</a>
                      <a class="dropdown-item" href="{{route('goodstoreout.show',['goodstoreout'=>$staff->id])}}">View</a>
                      <div class="dropdown-divider"></div>
                     
                      <button action="{{route('goodstoreout.destroy',['goodstoreout'=>$staff->id])}}" class="dropdown-item text-danger deleteListItem"  data-toggle="modal" data-target="#DeleteModel" type="submit">Delete</button>
                    </div>
                  </div>
                </div>
              </td>
