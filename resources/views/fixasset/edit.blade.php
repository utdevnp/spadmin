...
@section('content')
<div class="card">
  <h5 class="card-header">Update Fix Assets </h5>
  <div class="card-body bg-light overflow-hidden">
   <form method="post" action="{{route("fixasset.update",['fixasset'=>$fixasset->id])}}">
        @csrf
        @method('PUT')
        <div class="row">
        ...
        @livewire('multi-select-assets')
            ...
            <div class="form-group col-md-4">
                ...
                <select name="condition" class="custom-select">
                    ...
                    @foreach($asset_contions as $key => $value)
                        <option @if($fixasset->condition == $key) selected  @endif value="{{$key}}">{{$value}} </option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <select name="responsable_person" class="custom-select">
                    ...
                    @foreach($staffs as $staffs)
                        <option @if($fixasset->responsable_person == $staffs->id) selected  @endif  value="{{$staffs->id}}">{{$staffs->name}} </option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <select name="assets_location" class="custom-select">
                    ...
                    @foreach($asset_location as $key => $value)
                        <option @if($fixasset->assets_location == $key) selected  @endif  value="{{$key}}">{{$value}} </option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <input class="form-control datetimepicker" value="{{$fixasset->purchase_date}}" name="purchase_date" id="datepicker" type="text" placeholder="YYYY-MM-DD" data-options='{"dateFormat":"Y-m-d"}'/>
                @error('purchase_date')
                   <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <select name="supplier_id" class="custom-select">
                    ...
                    @foreach($suppliers as $supply)
                        <option @if($fixasset->supplier_id == $supply->id) selected  @endif  value="{{$supply->id}} ">{{$supply->name}} </option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <input class="form-control" value="{{$fixasset->quantity}}" name="quantity" type="number" placeholder="10" />
                @error('quantity')
                   <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <input class="form-control" value="{{$fixasset->rate}}" name="rate" type="number" placeholder="500" />
                @error('rate')
                   <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
