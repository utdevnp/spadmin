                ...
                @if(in_array($value['route_name'], getPermission()))
                  @if($value['child'])
                    <li class="nav-item dropdown dropdown-on-hover">
                      <a class="nav-link dropdown-toggle" id="navbarDropdownHome" href="{{route("dashboard")}}#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                      {{$value['name']}}
                      </a>
                      <div class="dropdown-menu dropdown-menu-card" aria-labelledby="navbarDropdownHome">
                        <div class="bg-white rounded-soft py-2">
                          ...
                          @foreach($value['childNav'] as $childKey => $childValue)
                            <a class="dropdown-item" href="{{route($childValue['link'])}}">{{$childValue['name']}}</a>
                          @endforeach
          ...
          <ul class="navbar-nav navbar-nav-icons ml-auto flex-row align-items-center">
            ...
            <li class="nav-item dropdown dropdown-on-hover"><a class="nav-link pr-0" id="navbarDropdownUser" href="{{route("dashboard")}}#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              ...
              <div class="dropdown-menu dropdown-menu-right py-0" aria-labelledby="navbarDropdownUser">
                <div class="bg-white rounded-soft py-2">
                  <a class="dropdown-item font-weight-bold text-warning" href="{{route("dashboard")}}"><span class="fas fa-desktop  mr-1"></span><span>Dashbaord</span></a>
